...
def getReportModel(timeframe, query, start_date, end_date, product):
    ...
    if timeframe == "Day":
        ...
        for p in ordersFound:
            total_price = round(p['p_price'] * p['product_quantity'], 2)
            total_cost = round(p['p_cost'] * p['product_quantity'], 2)
            earnings = round((float(p['p_price']) - float(p['p_cost'])) * int(p['product_quantity']), 2)
            results.append({'tracking_num': p['tracking_number'], 'p_name': p['p_name'], 'p_brand': p['p_brand'],
                            'order_date': p['order_date'], 'price': p['p_price'], 'cost': p['p_cost'],
                            'quantity': p['product_quantity'],
                            'total_price': total_price, 'total_cost': total_cost, 'earnings': earnings})
        return results
